...
export default function TeamRoomMemberPage() {
  ...
  // 팀룸 상세 조회 (profileImageId, role 정보)
  ...
  // 멤버 상세 조회 (email, major, mbti, joinedAt 정보)
  const { data: memberDetail } = useTeamMemberDetail(teamRoomId, memberIdNum);

  // 팀룸 멤버 기본 정보 (profileImageId, role)
  const memberBasic = teamRoom?.members.find((m) => m.userId === memberIdNum);

  // 현재 유저가 팀장인지 확인
  const leader = teamRoom?.members.find((m) => isLeader(m.role));
  const isCurrentUserLeader = leader?.userId === currentUserId;
  ...
  if (!memberBasic || !memberDetail) return null;

  return (
    <>
      <TopBar title="프로필 관리" />
      <MemberProfileSection
        avatar={`/assets/character/char-${memberBasic.profileImageId}.png`}
